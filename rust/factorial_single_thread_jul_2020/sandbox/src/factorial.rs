...
struct FactorialWorker {
    factorial_table: HashMap<u64,[u64; MAX_SIZE]>,
}

impl FactorialWorker {
    fn new() -> FactorialWorker {
        FactorialWorker {
            factorial_table: HashMap::new(),
        }
    }

    fn get_factorial(&mut self, n: u64) -> [u64;  MAX_SIZE] {
        if self.factorial_table.contains_key(&n) {
            // println!("TRACER cache hit!");
            *self.factorial_table.get(&n).unwrap()
        } else {
            let result = self.compute_factorial(n);
            self.factorial_table.insert(n, result);
            result
        }
    }
    ...
    fn compute_factorial(&self, n: u64) -> [u64;  MAX_SIZE] {
        ...
        for i in 0..n+1 {
            let factor = FactorizationWorker::new().get_factorization(i);
            for j in 0..MAX_SIZE {
                result[j] += factor[j];
            }
        }
...
struct FactorizationWorker {
    factor_table: HashMap<u64,[u64; MAX_SIZE]>,
}

impl FactorizationWorker {
    fn new() -> FactorizationWorker {
        FactorizationWorker {
            factor_table: HashMap::new(),
        }
    }

    fn get_factorization(&mut self, n: u64) -> [u64;  MAX_SIZE] {
        if self.factor_table.contains_key(&n) {
            // println!("TRACER cache hit!");
            *self.factor_table.get(&n).unwrap()
        } else {
            let result = self.compute_factorization(n);
            self.factor_table.insert(n, result);
            result
        }
    }
